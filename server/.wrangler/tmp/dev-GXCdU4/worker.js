...
// .wrangler/tmp/bundle-shVvzU/strip-cf-connecting-ip-header.js
...
function makeGuess(state, targetPlayerId, which, now = Date.now()) {
...
function finalizeResolution(state) {
  if (state.phase !== "turnResolve")
    return state;
...
function handleFinalizeResolution(room) {
  if (room.phase !== "playing" || !room.game)
    return room;
  if (room.game.phase !== "turnResolve")
    return room;
  return { ...room, game: finalizeResolution(room.game) };
}
__name(handleFinalizeResolution, "handleFinalizeResolution");
...
var GameRoom = class {
  ...
  async alarm() {
    ...
    if (!this.room)
      return;
    if (this.room.phase === "playing" && this.room.game?.phase === "turnResolve" && this.room.game.resolveAt !== null && Date.now() >= this.room.game.resolveAt - 50) {
  ...
  async syncAll() {
    ...
    if (this.room.phase === "playing" && this.room.game?.phase === "turnResolve" && this.room.game.resolveAt !== null) {
...
// .wrangler/tmp/bundle-shVvzU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-shVvzU/middleware-loader.entry.ts
